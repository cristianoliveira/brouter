...
failures stay visible and never silently open a different browser.
...
- **macOS:** discover generated Chromium-family/Brave and Chrome `.app`
  ...
  generated metadata first, then parse only the stable origin, scope, app ID,
  browser identity, and launch information that the browser actually emits.
  Launch through a structured platform adapter using an opaque bundle/app
...
- **Linux:** discover generated Brave and Chrome `.desktop` launchers in the
  supported XDG application locations. Parse desktop-entry fields with a
  desktop-entry parser, validate URL placeholders and argv safely, and reject
  shell expressions or arbitrary entries that do not identify a generated web
  app. Support the existing NixOS + Sway/Wayland baseline only; do not imply
...
- [ ] A shared catalog/launcher contract has contract tests, and real-shaped
      ...
      adapters at initial completion.
...
- [ ] Documentation states supported macOS and Linux baselines, generated-app
      limitations, refresh behavior, failure/fallback policy, and explicit
      non-support for Windows, app creation/installation, other engines, and
      default-handler changes.
