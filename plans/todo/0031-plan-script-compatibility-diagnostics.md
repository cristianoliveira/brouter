...
## Acceptance criteria
- [ ] Cover static-only regression, script defer/target/failure/timeout, unknown target, malformed URL, repeated events, fixed time, concurrency, and deterministic ordering.
...
- [ ] Define supported local/CI platform matrix, pinned fixtures, timeout/resource-boundary tests, and explicit unavailable/UNTESTED cases.
- [ ] Keep the matrix compatible with current CLI output/exit contracts and a no-script configuration.
