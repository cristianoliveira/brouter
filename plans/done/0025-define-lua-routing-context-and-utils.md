...
## Proposed initial helper surface
- ctx.utils.date(...): a restricted date-formatting adapter with explicitly documented supported signatures/formats.
- ctx.utils.time(...): a restricted time adapter with explicitly documented supported signatures, units and invalid-input behavior.
- No extra helper is included merely because the Lua standard library has it.
...
- Specify date/time semantics intentionally: do not promise full os.date/os.time compatibility before runtime support is known. Define local vs UTC behavior, invalid dates/formats and any unsupported overloads.
...
- Include examples with no time condition and small contract-test scenarios for fake time, supported formatting, invalid arguments and attempts to access forbidden capabilities. These are acceptance scenarios until a runtime exists, not claims that executable tests passed.
...
## Follow-up boundary
Lua embedding/configuration mode must be planned separately once chosen. Completion here approves the contract only; it does not mean Lua configurations work in brouter.
